...
export function savePersonalScore({ wpm, accuracy, timeTaken, mode, difficulty = null, consistency = null }) {
  const all = JSON.parse(localStorage.getItem(LS_KEY) || '[]')
  all.push({ wpm, accuracy, timeTaken, mode, difficulty, consistency, timestamp: Date.now() })
...
export function getStatsOverview() {
  ...
  const recent = all.filter(s => s.timestamp >= sevenDaysAgo)
